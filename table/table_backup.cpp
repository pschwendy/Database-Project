...
template <typename T>
JoinMap<T> Table::map_out(vector<pair<string, int> > select_columns, 
                    string &column,
                    int row_size) {
    JoinMap<T> join_map;
    ...
    for(size_t i = 0; i < table.rows_size(); ++i) {
        /*::database::Row join_row;
        join_row.mutable_entries()->Reserve(column_indecies.size());
        for(int j = 0; j < row_size; ++j) {
            join_row.add_entries();
        }

        for(pair<Info, int> placement_info: join_placements) {
            ::database::Entry* entry = join_row.mutable_entries(placement_info.second);
...
vector<::database::Row> Table::join(Table& table_compare, 
                                JoinMap<T>& join_map,
                                vector<pair<string, int> > select_columns,
                                vector<pair<string, int> > select_columns2,
                                string &column,
                                int row_size) {
    vector<::database::Row> subset;
    ...
    for(size_t i = 0; i < table.rows_size(); ++i) {
        ...
        for(pair<Info, int> placement_info: join_placements) {
            ::database::Entry* entry = table_compare.get_row(it->second).mutable_entries(placement_info.second);
            entry->CopyFrom(table.rows(i).entries(placement_info.first.index));
        }
        subset.emplace_back(it->second);
    }
